...
export default {
  ...
  methods: {
    ...
    async fetchTodos() {
      try {
        const response = await fetch('https://backend-todo-app-test-ue99.onrender.com/todos');  // Replace with your backend API URL
    ...
    async addTodo() {
      ...
      try {
        const response = await fetch('https://backend-todo-app-test-ue99.onrender.com/todos', {
          method: 'POST',
          headers: {
    ...
    async removeTodo(id) {
      try {
        const response = await fetch(`https://backend-todo-app-test-ue99.onrender.com/todos/${id}`, {
          method: 'DELETE'
        });
    ...
    async updateTodoStatus(todo) {
      try {
        const response = await fetch(`https://backend-todo-app-test-ue99.onrender.com/todos/${todo.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
